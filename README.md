# 🏛️ Sistema de Oposiciones TAI - Plataforma Integral de Preparación y Test
...
## 🚀 Demo Web en Vivo (1-Clic y Coste 0€)
...
1. **📖 Modo Estudio (Aprendizaje Ágil en Tiempo Real):** Validación instantánea en verde/rojo tras seleccionar cada respuesta, favoreciendo la asimilación activa de conceptos en los 4 Bloques del temario sin necesidad de esperar a la conclusión del test.
2. **⏱️ Modo Examen Oficial INAP (Temporizador y Baremación Real):** Simulación estricta de las condiciones de examen público: cuenta atrás visible, ocultación de soluciones durante el desarrollo y cálculo automático de la nota con el baremo oficial (**+1,0 punto por acierto, -0,33 puntos por fallo**, 0,0 por respuesta en blanco).
...
## 🏗️ Arquitectura Backend (.NET 10 + Dapper + T-SQL)

El cerebro backend aplica **Clean Architecture**, separando estrictamente los contratos del dominio, la capa de acceso a datos de altísimo rendimiento con **Dapper** y la exposición REST con **ASP.NET Core 10**:

```mermaid
graph TD
    CLIENT[🖥️ Cliente Web / Frontend Responsive] <-->|CORS Enabled| API[🌐 Oposiciones.Api - ASP.NET Core 10]
...
## 📝 Licencia y Autor
Diseñado y desarrollado profesionalmente por **[NainDev (Aitor Nain)](https://github.com/Nain9Dev)**. Construido como herramienta real para estudio competitivo y demostración técnica de Clean Architecture con .NET 10 y SQL Server.
